...
class _MessageAckState extends State<ChatMessageAckPage> {
    ...
    if (_state == _State.init) {
      ...
      return Column(
        children: [
          ...
          SvgPicture.asset('images/ic_message_read.svg', package: kPackage),
  ...
  void initState() {
    ...
    if (widget.ackInfo != null) {
      ...
      ChatMessageRepo.fetchTeamMessageReceiptDetail(widget.message).then((
        value,
      ) {
        Alog.d(
          tag: 'ChatKit',
  ...
  Widget build(BuildContext context) {
    ...
    final tabBar = TabBar(
      ...
      tabs: [
        ...
        Text(
          S.of(context).messageUnreadWithNumber(unreadList.length.toString()),
        ),
      ],
    );
    ...
    if (widget.isDialog) {
      return DefaultTabController(
        ...
        child: ClipRRect(
          ...
          child: Scaffold(
            appBar: AppBar(
              title: Text(S.of(context).messageReadStatus),
              centerTitle: true,
              elevation: 0.5,
