...
class SuperTooltip {
  ...
  void show(BuildContext targetContext) {
    ...
    if (!isTargetHeadVisible) {
      final dy = _targetCenter?.dy ?? 0;
      final dx = _targetCenter?.dx ?? 0;
      // 获取屏幕安全区域
      final mediaQuery = MediaQuery.of(targetContext);
      final screenHeight = mediaQuery.size.height;
      final visibleAreaTop = mediaQuery.padding.top;
      final visibleAreaBottom = screenHeight - mediaQuery.padding.bottom;
      //中心点不在屏幕
      if (dy < visibleAreaTop || dy > visibleAreaBottom) {
        if (popupDirection == TooltipDirection.up) {
          _targetCenter = Offset(dx, visibleAreaTop + 300);
          arrowTipDistance = 2.0;
        } else if (popupDirection == TooltipDirection.down) {
          _targetCenter = Offset(dx, visibleAreaBottom - 300);
          arrowTipDistance = 2.0;
        }
      }
    }
