...
#没有指定平台，说明不想出产物
if [ "$ios_distribute_platform" = "" ] ; then
...
echo "删除 meeting_kit下的podfile.lock"
if [[ -f "$project_path/../meeting_kit/example/ios/Podfile.lock" ]]; then
...
#指定输出归档文件地址
archive_path="$output_path_symbol/flutter_im_uikit_${export_method}_${now}.xcarchive"
archive_zip_path="$output_path_app/flutter_im_uikit_${export_method}_${now}.xcarchive.zip"
#指定输出ipa名称
